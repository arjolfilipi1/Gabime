        ...
        if request.method == 'GET':
            ...
            if slug:
                try:
                    ...
                    if d:
                        Gab.objects.create(cir=slug,lot = lot,grup=se.grup,pos=d,defect_code=defekt,oid=oid,lider_assy= assy,lider_sub=sub,masa= masa,cd= datetime.datetime.now(),eq=lloj,ac_code=ac_code)
                        Last = Gab.objects.order_by('-id')[:5]
